...
/** Map a DB location row to the API response shape */
...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const result = await pool.query(
            ...
            [tripId, req.userId, coords.lat, coords.lng, title, comment || '', photoUrl || '', type || 'adventure', timestamp || Date.now()]
        );
...
router.put('/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const result = await pool.query(
            ...
            [req.params.id, req.userId, title, comment, type, timestamp]
        );

        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Location not found or not owned by you' });
        }
